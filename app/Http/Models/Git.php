...
class GitRepo
{
  ...
  public function config($user_name, $user_email, $user_password)
  {
    $this->run('config user.name "' . $user_name . '"');
    $this->run('config user.email "' . $user_email . '"');
    $this->run('config user.password "' . $user_password . '"');
  }
}
